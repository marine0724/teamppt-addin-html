...
**Tech Stack:** C# / .NET Framework 4.8, Newtonsoft.Json, `System.Net.Http.HttpClient`, xUnit. z.ai 일반 OpenAI-호환 엔드포인트 `https://api.z.ai/api/openai/v1/chat/completions` (무료 티어, `Authorization: Bearer <key>`).
...
| `Services/GlmSchema.cs` (신규) | Gemini responseSchema → z.ai json_schema 정규화 | 3 |
...
z.ai json_schema는 표준 JSON Schema(소문자 type)를 받는다. 우리 스키마는 이미 소문자(`object`/`string`/`integer`/`number`/`array`)라 거의 1:1이지만, 미래에 Gemini식 대문자(`STRING`)가 섞여도 안전하도록 소문자 정규화 + 입력 비파괴(clone)를 보장한다.
...
namespace TeampptAddin
{
    /// <summary>Gemini responseSchema(JObject)를 z.ai json_schema로 정규화. 입력 비파괴.</summary>
...
public class GlmRequestBuilderTest
{
    ...
    [Fact]
    public void ResponseFormat_IsJsonSchema_WithNormalizedSchema()
    {
        var body = GlmRequestBuilder.Build("sys", "hi", null, Schema(), 0.5, 0);
        Assert.Equal("json_schema", body["response_format"]["type"].ToString());
        Assert.Equal("object",
            body["response_format"]["json_schema"]["schema"]["type"].ToString());
    }
...
`[BUILD-LOGIC]` → `[TEST]` (`--filter "FullyQualifiedName~GlmRequestBuilderTest"`). 기대: 5개 PASS.

- [ ] **Step 5: 커밋**

```bash
git add src/TeampptAddin/Services/GlmRequestBuilder.cs src/TeampptAddin.Tests/GlmRequestBuilderTest.cs
git commit -m "feat(glm): GlmRequestBuilder 요청 바디(모델 자동선택·json_schema·thinking) (Task 4, TDD 5/5)"
```
...
namespace TeampptAddin
{
    ...
    {
        private const string Endpoint = "https://api.z.ai/api/openai/v1/chat/completions";
...
순수 로직은 검증됐지만 z.ai 실호출(특히 비전 `glm-4.6v-flash`의 json_schema 수용 여부)은 실제 호출로만 확인된다. 관리자 빌드 후 실제 덱으로 텍스트·비전 경로를 한 번씩 태운다.
...
- `[GLM] 토큰: ...`
기대: 컨셉 3안이 정상 표시(파싱 성공). HTTP 4xx(특히 json_schema 거부)면 Step 6 폴백 적용.
...
- `HTTP 200` + 정상 파싱
기대: 비전 응답이 스키마대로 파싱됨. 만약 `glm-4.6v-flash`가 `response_format=json_schema`를 거부(4xx)하면 Step 6 폴백.
...
- [ ] **Step 6: (조건부) json_schema 거부 시 폴백**

Step 3/4에서 어느 모델이든 `json_schema`를 거부하면, `GlmRequestBuilder.Build`의 `response_format`을 `json_object`로 바꾼다(스키마는 이미 system 프롬프트에 포함돼 있어 파싱 유지됨):
```csharp
                ["response_format"] = new JObject { ["type"] = "json_object" },
```
이때 `GlmRequestBuilderTest`의 `ResponseFormat_IsJsonSchema_WithNormalizedSchema` 테스트도 `json_object` 기대로 수정. `[BUILD-LOGIC]`→`[TEST]`로 그린 확인 후 Step 2부터 재검증. (거부가 없으면 이 스텝 건너뜀.)
...
**알려진 리스크/메모:**
- z.ai 무료 비전 모델의 `json_schema` 정확 수용 여부는 라이브 전까지 미확정 → Task 7 Step 6 폴백 경로로 흡수.
