...
| 6 | **Placeholder·그림삽입(Picture Placeholder) 사용 금지** | PPT COM의 Shape.Copy가 Placeholder의 Layout/Master 참조를 복사하지 못함 → 서식·이미지 유실. 일반 텍스트박스·도형·이미지로 대체 |
| 7 | **배경 이미지는 도형으로** — 슬라이드 배경 채우기(Background Fill) 대신 슬라이드 크기 사각형에 이미지를 넣는다 | Background Fill은 슬라이드 속성이라 Shape.Copy로 복사 불가. 도형이면 다른 shape과 함께 정상 복사됨 |

> **규약 6·7 배경**: 에셋은 `Shapes.Range.Copy()` → `Paste()`로 타겟 슬라이드에 삽입된다.
> 이 방식은 일반 shape(텍스트박스, 도형, 이미지, 그룹)은 완벽히 복사하지만,
> Placeholder shape와 슬라이드 배경 채우기는 복사할 수 없다 (PPT COM 근본 제약).
> 따라서 에셋 제작 시 모든 요소를 일반 shape으로 구성해야 한다.
